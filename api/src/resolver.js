  ...
  Query: {
      ...
      if (args.onlyData) {
        ...
        const relatedProducts = products.filter((item) => {
          return item.category.find((cat) =>
            product.category.find((c) => c.slug == cat.slug)
          );
        });
    ...
    products: async (root, args, ctx, info) => {
      ...
      let filteredProducts = products.filter((product) => {
        ...
        if (flag && args.brand && args.brand.length) {
          flag = product.brands.find((brand) =>
            args.brand.find((b) => b == brand.slug)
          );
        }
        ...
        if (flag && (args.color.length || args.size.length)) {
          flag = product.variants.find(
            (variant) =>
              (!args.color.length ||
                (variant.color &&
                  args.color.find((item) => item == variant.color_name))) &&
              (!args.size.length ||
                (variant.size &&
                  variant.size.find((item) =>
                    sizeArray.find((sa) => sa == item.name)
                  )))
          );
        }
          ...
          if (product.variants.length > 0) {
            for (let i = 0; i < product.variants.length; i++) {
              if (
                ...
                product.variants[i].price < maxPrice
              )
                flag = true;
            }
      ...
      return {
        data: filteredProducts.slice(
          args.from + (page - 1) * args.perPage,
          args.from + page * args.perPage
        ),
    ...
    postsByPage: async (root, args, ctx, info) => {
      ...
      if (args.category) {
        posts = posts.filter((post) =>
          post.blog_categories.find((item) => item.slug == args.category)
        );
      }

      const categories = require("./data/blog-page.json")["categories"][
        args.page
      ];
    ...
    post: async (root, args, ctx, info) => {
      ...
      const relatedPosts = blog.filter((item) => {
        return item.blog_categories.find((cat) =>
          post.blog_categories.find((c) => c.slug == cat.slug)
        );
      });
