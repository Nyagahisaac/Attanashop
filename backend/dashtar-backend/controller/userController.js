...
const verifyEmailAddress = async (req, res) => {
  const isAdded = await users.findOne({ email: req.body.email });
  if (isAdded) {
...
const loginUser = async (req, res) => {
  try {
    const user = await users.findOne({ email: req.body.registerEmail });
...
const forgetPassword = async (req, res) => {
  const isAdded = await users.findOne({ email: req.body.verifyEmail });
  if (!isAdded) {
...
const deleteUser = async (req, res) => {
  ...
  const response = await users.destroy({ where: { id: newId } })
