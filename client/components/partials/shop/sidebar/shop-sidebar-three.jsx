...
function ShopSidebarThree ( props ) {
    ...
    function onAttrClick ( e, attr, value , brandname) {
        if ( getUrlForAttrs( attr, value ) )
            router.push( getUrlForAttrs( attr, value, brandname ) );
    }
                            ...
                            <div className="filter-items filter-items-count">
                                {
                                    shopData.categories.map( ( item, index ) =>
                                        <div className="filter-item" key={ `cat_${index}` }>
                                            <ALink className={ `${query.category == item.id ? 'active' : ''}` } href={ { pathname: router.pathname, query: { category: item.id } } } scroll={ false }>{ item.name }</ALink>
